...
class Game {
    ...
    addHero (distanceFromTop, distanceFromLeft, name, team, id) {
        ...
        // Can only add a hero to unoccupied spaces or spawn points
        if (this.board.tiles[distanceFromTop][distanceFromLeft].type === 'Unoccupied' || 
        this.board.tiles[distanceFromTop][distanceFromLeft].type === 'SpawnPoint') {
    ...
    // Adds a spawn point to the board
    // team values: 1, 2, or A (Any)
    addSpawnPoint (distanceFromTop, distanceFromLeft, team) {
        if (this.hasStarted) {
            throw new Error('Cannot add spawn point after the game has started!');
        }

        // Can only add a spawn point to unoccupied spaces
        if (this.board.tiles[distanceFromTop][distanceFromLeft].type === 'Unoccupied') {
            const spawnPoint = new SpawnPoint(distanceFromTop, distanceFromLeft, team);
            this.board.tiles[distanceFromTop][distanceFromLeft] = spawnPoint;
            switch (team) {
            case '1':
                this.team1SP.push(spawnPoint);
                break;
            case '2':
                this.team2SP.push(spawnPoint);
                break;
            case 'A':
                this.anyTeamSP.push(spawnPoint);
                break;
            default:
                // Do nothing
            }
    ...
    hasSpawnPointsLeft (team) {
        if (team === 0) {
            return this.team1SP.length > 0 || this.anyTeamSP.length > 0;
        }
        return this.team2SP.length > 0 || this.anyTeamSP.length > 0;
    }

    getNextSpawnPoint (team) {
        if (team === 0) {
            if (this.team1SP.length > 0) {
                return this.team1SP.pop();
            }
            if (this.anyTeamSP.length > 0) {
                return this.anyTeamSP.pop();
            }
            // Shouldn't ever throw as long as it is validated first.
            throw new Error(`No more Spawn Points for team ${team}`);
        }

        if (this.team2SP.length > 0) {
            return this.team2SP.pop();
        }
        if (this.anyTeamSP.length > 0) {
            return this.anyTeamSP.pop();
        }
    ...
    hasValidSpawnPoints (maxPlayersPerTeam) {
        return maxPlayersPerTeam >= this.team1SP.length &&
            maxPlayersPerTeam >= this.team2SP.length &&
            maxPlayersPerTeam * 2 >= this.team1SP.length + this.team2SP.length + this.anyTeamSP.length;
    }

    // Free any unused spawn points
    flushSpawnPoints () {
        for (let i = 0; i < this.team1SP.length; i++) {
            const spawnPoint = this.team1SP[i];
            this.board.tiles[spawnPoint.distanceFromTop][spawnPoint.distanceFromLeft] =
                new Unoccupied(spawnPoint.distanceFromTop, spawnPoint.distanceFromLeft);
        }
        for (let i = 0; i < this.team2SP.length; i++) {
            const spawnPoint = this.team2SP[i];
            this.board.tiles[spawnPoint.distanceFromTop][spawnPoint.distanceFromLeft] =
                new Unoccupied(spawnPoint.distanceFromTop, spawnPoint.distanceFromLeft);
        }
        for (let i = 0; i < this.anyTeamSP.length; i++) {
            const spawnPoint = this.anyTeamSP[i];
            this.board.tiles[spawnPoint.distanceFromTop][spawnPoint.distanceFromLeft] =
                new Unoccupied(spawnPoint.distanceFromTop, spawnPoint.distanceFromLeft);
        }
    }
