  ...
  // Increment the game turn and update the active hero
    ...
    if (this._teamIsDead(this.teams[0])) {
        ...
        if (teamDiamonds1 > teamDiamonds0) {
            this.winningTeam = 1;
        } else if (teamDiamonds1 < teamDiamonds0) {
            this.winningTeam = 0;
        } else {
...
Game.prototype.filter = {};
Game.prototype.filter.onlyLivingHeroes = function (hero) {
    return hero.dead === false;
};

Game.prototype.filterTeamHeroes = function (filterFn) {
    let filtered = [];
...
Game.prototype.getIdOfTeamWithMostLivingHeroes = function () {
    let livingHeroesByTeam = this.filterTeamHeroes(this.filter.onlyLivingHeroes);
    let id = -1;

    livingHeroesByTeam.reduce((previousTeamHeroCount, team, teamId) => {
        if (team.length > previousTeamHeroCount) {
            id = teamId;
        }

        return team.length;
    }, livingHeroesByTeam[0]);
