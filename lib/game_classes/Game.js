...
class Game {
    ...
    */
    addHero (distanceFromTop, distanceFromLeft, name, team, id) {
        ...
        let tile = this.board.tiles[distanceFromTop][distanceFromLeft];
    ...
    addHeroToGame (distanceFromTop, distanceFromLeft, name, team, id) {
    ...
    addSpawnPoint (distanceFromTop, distanceFromLeft, spawnValue) {
        if (this.hasStarted) {
            throw new Error('Cannot add impassables after the game has started!');
        }
        
        const currentTile = this.board.tiles[distanceFromTop][distanceFromLeft];

        // Can only add an spawn point to unoccupied spaces are not spawn points 
        if (currentTile.type === 'Unoccupied' && currentTile.subType !== 'SpawnPoint') {

            // Creates new Unoccupied object with spawn value
            const spawnPoint = new Unoccupied(distanceFromTop, distanceFromLeft, spawnValue);
