...
// started
Game.prototype.addHero = (distanceFromTop, distanceFromLeft, name, team, id) => {
    if (this.hasStarted) {
  ...
  // Can only add a hero to unoccupied spaces
    ...
    // Creates new hero object
        let hero = new Hero(distanceFromTop, distanceFromLeft, name, team);
...
Game.prototype.addDiamondMine = (distanceFromTop, distanceFromLeft) => {
    if (this.hasStarted) {
  ...
  // Can only add a diamond mine to unoccupied spaces
    ...
    // Creates new diamond mine object
        let diamondMine = new DiamondMine(distanceFromTop, distanceFromLeft);
...
Game.prototype.addHealthWell = (distanceFromTop, distanceFromLeft) => {
    if (this.hasStarted) {
  ...
  // Can only add a health well to unoccupied spaces
    ...
    // Creates new health well object
        let healthWell = new HealthWell(distanceFromTop, distanceFromLeft);
...
Game.prototype.addImpassable = (distanceFromTop, distanceFromLeft) => {
    if (this.hasStarted) {
  ...
  // Can only add an impassable to unoccupied spaces
    ...
    // Creates new impassable object
        let impassable = new Impassable(distanceFromTop, distanceFromLeft);
...
Game.prototype.handleHeroTurn = (direction) => {
    if (this.ended) {
        return;
    }
  ...
  // Increment the game turn and update the active hero
    ...
    if (this._teamIsDead(this.teams[0])) {
        ...
        this.ended = true;
        let teamDiamonds0 = this._teamDiamonds(this.teams[0]);
        let teamDiamonds1 = this._teamDiamonds(this.teams[1]);
...
Game.prototype._handleHeroEarnings = (hero) => {
    if (hero.mineCount > 0) {
...
Game.prototype._handleHeroMove = (hero, direction) => {
    this.moveMessage = hero.name + ' walked ' + direction;

  // Gets the tile at the location that the hero wants to go to
    let tile = this.board.getTileNearby(hero.distanceFromTop, hero.distanceFromLeft, direction);
...
Game.prototype._resolveHeroAttacks = (hero) => {
  ...
  // Loop through all tiles around the hero
    for (let i=0; i<directions.length; i++) {
        let tile = this.board.getTileNearby(hero.distanceFromTop, hero.distanceFromLeft, directions[i]);
        if (tile === false) {
...
Game.prototype._teamDiamonds = (teamArray) => {
    let diamonds = 0;
...
Game.prototype._teamIsDead = (teamArray) => {
    for (let i=0; i<teamArray.length; i++) {
        if (!teamArray[i].dead) {
            return false;
        }
...
Game.prototype._incrementTurn = () => {

  // Used to determine whose turn it is
    let incrementHeroTurnIndex = () => {
        this.heroTurnIndex++;
...
Game.prototype.heroDied = (hero) => {

  // Removes a dead hero from the board
    let top = hero.distanceFromTop;
    let left = hero.distanceFromLeft;
    let bones = new Unoccupied(top, left);
    let teamName = (hero.team === 1) ? 'Red' : 'Blue';
